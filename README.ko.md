...
| `vch test [<name>]` | `xcodebuild test` 실행 시 `-resultBundlePath` 주입. 작업 worktree 안에서는 `<name>` 생략 가능; 시뮬레이터는 지연 클론 (`--device`, `--project`, `--workspace`, `--runtime`, `--only-testing`, `--skip-testing`, `--rerun`, `--rerun-failed`, `--erase-clone`, `--shutdown-template`). |
