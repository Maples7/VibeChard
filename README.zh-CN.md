...
| `vch test [<name>]` | 跑 `xcodebuild test`，注入 `-resultBundlePath`；在任务 worktree 内可省略 `<name>`，懒克隆模拟器（`--device`、`--project`、`--workspace`、`--runtime`、`--only-testing`、`--skip-testing`、`--rerun`、`--rerun-failed`、`--erase-clone`、`--shutdown-template`）。 |
