...
| `vch remove <name> [--allow-dirty] [--allow-unmerged] [--keep-sim]` | Delete the worktree, branch, and (by default) simulator clone. `--allow-dirty` permits uncommitted changes; `--allow-unmerged` force-deletes a branch that isn't fully merged. `-f / --force` is a deprecated alias (= both flags) and emits a stderr warning; removed in 1.0. |
...
flags; it is removed in 1.0. Per-task simulator clones are also
renamed in v0.3.0: `<original>-vch-<task>` instead of
