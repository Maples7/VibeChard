...
穩定版本提供預先編譯的 Homebrew bottle，支援 Apple Silicon 的 macOS 14+
和 Intel 的 macOS 15+。有相容 bottle 時，Homebrew 不必編譯 vch，也不會
觸發原始碼建置的 Xcode 版本檢查。其他組態及 `--HEAD` 仍從原始碼建置；
若升級 macOS 後，Homebrew 因 Xcode 版本較舊而拒絕安裝，可依下方步驟
手動從原始碼安裝。建置 Apple 專案仍需要 Xcode。
...
| `vch test [<name>]` | 跑 `xcodebuild test`，注入 `-resultBundlePath`；在任務 worktree 內可省略 `<name>`，懶克隆模擬器（`--device`、`--project`、`--workspace`、`--runtime`、`--only-testing`、`--skip-testing`、`--rerun`、`--rerun-failed`、`--erase-clone`、`--shutdown-template`、`--test-execution-idle-timeout`、`--progress-interval`）。 |
